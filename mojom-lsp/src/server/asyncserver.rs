...
use super::definition::find_definition_preorder;
use super::document_symbol::{get_document_symbols, get_identifier, DocumentSymbol};
...
where
    ...
    W: AsyncWrite + Unpin,
{
    let message = asyncrpc::recv_message(reader).await?;
    ...
    W: AsyncWrite + Unpin,
{
    while let Some(message) = receiver.recv().await {
        asyncrpc::send_message(&mut writer, &message).await?;
    }
    Ok(())
}
...
impl ParsedMojom {
    fn find_definition(&self, position: &lsp_types::Position) -> Option<lsp_types::Location> {
        let ident = get_identifier(&self.ast.text, position);
        self.find_definition_in_text(&ident)
            .or(self.find_definition_in_imports(&ident))
    }

    fn find_definition_in_text(&self, ident: &str) -> Option<lsp_types::Location> {
        find_definition_preorder(ident, &self.ast)
    }
    ...
    R: AsyncBufRead + Unpin,
{
    async fn run(mut self) -> anyhow::Result<i32> {
        loop {
            let message = asyncrpc::recv_message(&mut self.reader).await?;
            match message {
...
async fn parse_mojom(
    ...
    let module = super::semantic::find_module(&text, &mojom, diagnostics);
    ...
    for import in imports {
        ...
        let module_name = {
            ...
            if modules.len() == 0 {
                ...
                let name = imported_text[module.name.start..module.name.end].to_string();
                Some(name)
            } else {
                log::debug!("Multiple module statements: {}", import_path);
                None
            }
        };
        ...
        let symbols = get_document_symbols(&imported_text, &imported_mojom);
    ...
    let ast = MojomAst::from_mojom(uri, text, mojom, module);
